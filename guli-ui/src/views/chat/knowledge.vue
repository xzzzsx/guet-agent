<template>
  <div class="app-container">
    ...
    <el-dialog title="文件内容内下" :visible.sync="dialogOpen" width="550px">
  ...
  methods: {
    getProjectList() {
      listProject().then((response) => {
        ...
        this.form.projectId =
          this.formProjectList.length == 0
            ? undefined
            : this.formProjectList[0].projectId;
      });
    },
    /** 查询岗位列表 */
    getList() {
    ...
    handleDelete(row) {
      ...
      const knowledgeId = row.knowledgeId;
      this.$modal
        .confirm('是否确认删除知识库编号为"' + knowledgeId + '"的数据项？')
        .then(function () {
